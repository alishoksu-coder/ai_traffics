# backend/app/vehicles.py вЂ” СЃРёРјСѓР»СЏС†РёСЏ Р°РІС‚РѕР±СѓСЃРѕРІ Рё РјР°С€РёРЅ РІРґРѕР»СЊ РјР°СЂС€СЂСѓС‚РѕРІ
import json
import math
import random
import threading
...
def _interpolate_polyline(points: List[List[float]], progress: float) -> tuple[float, float]:
    """progress 0..1 в†’ (lat, lon) РІРґРѕР»СЊ Р»РёРЅРёРё."""
    if not points:
        return 51.16, 71.45
    n = len(points)
    if n == 1:
...
class VehicleSimulator:
    """Р”РµСЂР¶РёС‚ СЃРїРёСЃРѕРє С‚СЂР°РЅСЃРїРѕСЂС‚Р° (Р°РІС‚РѕР±СѓСЃС‹/РјР°С€РёРЅС‹), РґРІРёРіР°РµС‚ РёС… РІРґРѕР»СЊ СЃРµРіРјРµРЅС‚РѕРІ РґРѕСЂРѕРі."""
    ...
    def _spawn_vehicles(self) -> None:
        ...
        self._next_id = 1
        # РђРІС‚РѕР±СѓСЃС‹ (РёРјРёС‚Р°С†РёСЏ РіРѕСЂРѕРґСЃРєРѕРіРѕ С‚СЂР°РЅСЃРїРѕСЂС‚Р° РђСЃС‚Р°РЅС‹)
        for _ in range(14):
    ...
    def snapshot(self) -> List[Dict[str, Any]]:
        """РўРµРєСѓС‰РёРµ РїРѕР·РёС†РёРё С‚СЂР°РЅСЃРїРѕСЂС‚Р°: {id, type, lat, lon, route_name}."""
        segs = self._get_segments()
        seg_by_id = {int(s.get("id", 0)): s for s in segs}
        result = []
        with self._lock:
